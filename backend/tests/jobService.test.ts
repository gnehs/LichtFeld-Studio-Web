...
describe("jobService disk status", () => {
  ...
  it("stores a signal-based failure message when the process closes without an exit code", async () => {
    vi.resetModules();

    const root = fs.mkdtempSync(path.join(os.tmpdir(), "lfs-job-service-signal-"));
    const datasetsDir = path.join(root, "datasets");
    const outputsDir = path.join(root, "outputs");
    const logsDir = path.join(root, "logs");
    const dbPath = path.join(root, "db", "app.db");
    ...
    expect(failedJob?.errorMessage).toBe("Process terminated by signal SIGKILL");
  });
