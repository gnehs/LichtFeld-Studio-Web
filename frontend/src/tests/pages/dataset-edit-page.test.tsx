...
it("renders loaded previews with dataset file URLs and preview modes", async () => {
  ...
  expect(container.textContent).toContain("預覽模式");
  expect(container.textContent).toContain("CSS Mask");
  expect(container.textContent).toContain("疊圖");
  expect(container.textContent).toContain("編輯資料集");
  expect(container.textContent).toContain("刪除資料集");
  ...
  expect(maskPreview?.getAttribute("style") ?? "").not.toContain("mask-mode: luminance");

  const editButton = Array.from(container.querySelectorAll("button")).find(
    (button) => button.textContent?.trim() === "編輯資料集",
  );
