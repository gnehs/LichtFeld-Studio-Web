...
- 若你是手動放置資料夾，請先確認格式正確，再透過既有 dataset 註冊流程加入清單；前端只會顯示已經註冊成功的 dataset
- 前端建立任務頁可直接拖移 `.zip` 檔案進行上傳；ZIP 進入背景處理後，仍可先調整任務參數與名稱
- 若使用既有資料集模式，畫面上的「重新整理」只會重新抓取已註冊 dataset 清單，不會自動掃描未註冊的資料夾
